<?php 
    ...
    if(!empty($_POST)){
        ...
        $sql = "SELECT * FROM tb_cliente WHERE email = '$login' AND senha = '".sha1(md5($senha))."'";
        
        $rs = mysqli_query($conecta,$sql);
        
        if(mysqli_num_rows($rs) > 0){
            $sql = "SELECT * FROM tb_cliente WHERE habilita = 'a' AND email = '$login' AND senha = '".sha1(md5($senha))."'";
            ...
            if(mysqli_num_rows($rs1) > 0){
                ...
                $_SESSION['tempo']= time();
                $_SESSION['categoria']= "area-cliente";
